...
class PartialParse(object):
    ...
    def parse_step(self, transition):
        ...
        if transition == 'S' and len(self.buffer) > 0 :
            # 进行shift操作，buffer列表第一个元素进入stack列表尾部
            self.stack.append(self.buffer.pop(0))
            # self.stack.append(self.buffer[0])
            # self.buffer.pop(0)
        if transition == 'LA' and len(self.stack) > 1:
            # 进行left arc操作，dep列表加入左弧，stack列表去除倒数第二的元素
            self.dependencies.append((self.stack[-1],self.stack[-2]))
            self.stack.pop(-2)
        if transition == 'RA' and len(self.stack) > 1:
            # 进行right arc操作，dep列表加入右弧，stack列表去除倒数第一的元素
...
def minibatch_parse(sentences, model, batch_size):
    ...
    ### TODO:
    # 依次将数据中的每一个句子输入分段解析器，变为一系列解析器对象
    partial_parses = [PartialParse(sentence) for sentence in sentences]
    # 浅拷贝partial_parses,这样unfinished_parses里面的修改也会作用到partial_parses上
    unfinished_parses = partial_parses[:] 
   
    while len(unfinished_parses) > 0:
        # 截取batchsize数量的parse进行操作
        batch_parses = unfinished_parses[:batch_size]
        # 使用模型对解析器进行解析，生成下一步的动作
        transitions = model.predict(batch_parses)
        for i,p in enumerate(batch_parses):
            # 每个解析器使用模型返回的动作，进行一步解析操作
            p.parse_step(transitions[i])
            # 当这个解析器的栈只有root，并且buffer里面没有元素的时候，说明这个解析器已经完成
            #  移出待解析列表
            if  len(p.buffer) == 0 and len(p.stack) == 1:
                unfinished_parses.remove(p)
  
    
    # 最后将所有解析器中的dep取出，按照顺序返回
