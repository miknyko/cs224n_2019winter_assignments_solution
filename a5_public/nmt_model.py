...
class NMT(nn.Module):
    ...
    def forward(self, source: List[List[str]], target: List[List[str]]) -> torch.Tensor:
        ...
        if self.charDecoder is not None:
            ...
            # print(f'[INFO] target_padded_Chars size is {target_padded_chars.size()}')
            # print(f'[INFO] max_word_len is {max_word_len}')

            target_words = target_padded[1:].contiguous().view(-1)
            # print(f'[INFO] target_padded_Chars size is {target_padded_chars[1:].size()}')
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
        ...
        X = torch.nn.utils.rnn.pack_padded_sequence(X,source_lengths)  # (a PackedSequence object),rnn都接受这种封装的数据

        enc_hiddens,(h_n,c_n) = self.encoder(X)
        enc_hiddens = torch.nn.utils.rnn.pad_packed_sequence(enc_hiddens,batch_first=True)[0]
       
        h_n = h_n.permute(1,0,2).reshape(batch_size,-1)   # 整理h_n至(batch_size,2 * hidden_size),因为是正反两个方向
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        for Y_t in torch.split(Y,1):
            ...
            dec_state,o_t,_ = self.step(Ybar_t,dec_state,enc_hiddens,enc_hiddens_proj,enc_masks)  # 注意更新dec_State,这个bug de了两天
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        O_t = self.dropout(torch.tanh(V_t)) # Pay attention if you run sanity check, set the training to False
