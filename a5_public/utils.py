...
def pad_sents_char(sents, char_pad_token):
    ...
    ### TODO:
    batch_max_length = max([len(sent) for sent in sents]) #获得此batch里面最长句的长度
    sents_padded = sents.copy()
    for sent in sents_padded: # 遍历该batch    
        for i in range(batch_max_length - len(sent)): # 每一句都扩展相应长度的空白字
            sent.append([char_pad_token] * max_word_length)
        for word in sent:
            if len(word) > max_word_length:
                word = word[:max_word_length]                             
            for i in range(max_word_length - len(word)):        # 每一句里面的每一个词都拓展相应长度的空白字符编码
...
def pad_sents(sents, pad_token):
    ...
    sents_padded = sents.copy()
    batch_max_length = max([len(sent) for sent in sents])
    for sent in sents_padded:
        for i in range(batch_max_length - len(sent)):
